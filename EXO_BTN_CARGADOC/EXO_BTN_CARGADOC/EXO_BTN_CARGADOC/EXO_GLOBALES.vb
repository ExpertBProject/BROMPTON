...
#Region " Serie o Num Documento"
                        ...
                        Else
                            ...
                            Else
                                objglobal.SBOApp.StatusBar.SetText("(EXO) - No se ha encontrado serie para el documento.", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                                Dim sAnno As String = Year(oForm.DataSources.DataTables.Item(sData).GetValue("F. Documento", i).ToString)
                                sSQL = "SELECT ""Series"" "
                                sSQL += " FROM ""NNM1"" "
                                sSQL += " WHERE ""ObjectCode""=" & sTipoFac & " and ""Indicator""='" & sAnno & "' "
                                oRsSerie = CType(oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset), SAPbobsCOM.Recordset)
                                oRsSerie.DoQuery(sSQL)
                                If oRsSerie.RecordCount > 0 Then
                                    Dim sSerieDoc As String = oRsSerie.Fields.Item("Series").Value.ToString
                                    oDoc.Series = sSerieDoc
                                Else
                                    objglobal.SBOApp.StatusBar.SetText("(EXO) - No se ha encontrado serie para el documento y año.", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                                    Exit Function
                                End If
                            End If
                        End If
...
#End Region
                            ...
                            If oRsLin.Fields.Item("U_EXO_DOCTYPE").Value.ToString = "I" Then
                                ...
                                If sImpuesto = "" Then
                                    ...
                                    sImpuesto = EXO_GLOBALES.GetValueDB(objglobal.compañia, """OITM""", """VatGourpSA""", """ItemCode""='" & oRsLin.Fields.Item("U_EXO_ART").Value.ToString & "'")
                                End If
                    ...
                    While Not MyReader.EndOfData
                        Try
                            ...
                            If oRs.RecordCount > 0 Then
                                ...
                                Dim sTransSantander As String = "" : Dim sTransfY As String = "" : Dim sTransMM As String = "" : Dim sNUMMOVIL As String = "" : Dim sMARCA As String = ""
                                    ...
                                    For L = 1 To sCamposL.GetUpperBound(0)
                                        Select Case sCamposL(L, 1)
                                            ...
                                            Case "EXO_NUMMOVIL"
                                                ...
                                                End If
                                            Case "EXO_MARCA"
                                                sMARCA = Leer_Campo(sCamposL(L, 1), sCamposL(L, 3), sCamposL(L, 2), scampos, oSboApp)
                                                If sCamposL(L, 2) = "Y" And sMARCA = "" Then
                                                    Exit Sub
                                                End If
                                        End Select
                                    Next
...
#Region "Comprobar datos línea"
                                    ...
                                    If sTipoLineas = "I" Then
                                        ...
                                        If sArt = "" Then
                                            ...
                                            If sMARCA <> "" Then
                                                sSQL &= " and ""FrgnName"" like '%" & sMARCA & "%' "
                                            End If
                                            sArt = objglobal.refDi.SQL.sqlStringB1(sSQL)
                                            If sArt = "" Then
                                                sSQL = "SELECT ""ItemCode"" FROM ""OITM"" WHERE ""ItemName"" like '%Recarga " & sprecio.Replace(".", ",").Replace("-", "") & "%'"
                                                If sMARCA <> "" Then
                                                    sSQL &= " and ""FrgnName"" like '%" & sMARCA & "%' "
                                                End If
                                                sArt = objglobal.refDi.SQL.sqlStringB1(sSQL)
...
#End Region
                                    ...
                                    sSQL &= EXO_GLOBALES.DblNumberToText(objglobal, sprecio.Replace("-", "").ToString) & "," & EXO_GLOBALES.DblNumberToText(objglobal, sDtoLin.ToString)
